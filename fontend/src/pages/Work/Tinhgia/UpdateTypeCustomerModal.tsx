...
const UpdateTypeCustomerModal = ({open,onClose, tittle}: props) => {

    const [typeCustomer, setTypeCustomer] = useState<TypeCustomer[]>([]);
    const [selectID, setSelectID] = useState<number>(0);

    const selectDieCutting = typeCustomer.find((m)=> m.id === selectID);
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (resTypeCustomer.data.data.length > 0) {
                    setSelectID(resTypeCustomer.data.data[0].id);
                }
    ...
    const handleUpdate = ()=>{
            ...
            try {
                const ma = typeCustomer.find((m) => m.id === selectID) as TypeCustomer;
                TypeCustomerService.updateTypeCustomer(ma).then((response) => {
            ...
            <Box sx={style}>
                ...
                <span>
                  Giá(vnđ) <span style={{color: "red"}}>*</span> :
                            ...
                            <input
                                value={selectDieCutting?.precentage || 0}
                                type="number"
                                className="shadow"
                                onChange={(e) => {
                                    const newPrice = Number(e.target.value);
                                    setTypeCustomer((prev) =>
                                        prev.map((p) =>
                                            p.id === selectID ? { ...p, precentage: newPrice } : p
                                        )
                                    );
                                }}
                            ></input>
...
export default UpdateTypeCustomerModal;

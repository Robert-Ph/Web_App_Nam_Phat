...
const UpdateDieCuttiingModal = ({open,onClose, tittle}: props) => {

    const [dieCutting, setDieCutting] = useState<DieCutting[]>([]);
    const [selectID, setSelectID] = useState<number>(0);

    const selectDieCutting = dieCutting.find((m)=> m.id === selectID);
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (resDieCutting.data.data.length > 0) {
                    setSelectID(resDieCutting.data.data[0].id);
                }
    ...
    const handleUpdate = ()=>{
            ...
            try {
                const ma = dieCutting.find((m) => m.id === selectID) as DieCutting;
                DieCuttingService.updateDieCutting(ma).then((response) => {
...
export default UpdateDieCuttiingModal;
